...
def ODE_LCDM(x, x_prime, N_p, Om_m_0, g_a):
    ...
    g_eff = 1 + g_a*(1-torch.exp(N))**n - g_a*(1-torch.exp(N))**(2*n)
    
    res1 = diff(x, N_p) - x_prime
    res2 = diff(x_prime, N_p) + (x_prime)**2 - (3*torch.exp(N)/(2*a_eq*(1+(torch.exp(N)/a_eq)+alpha*(torch.exp(N)/a_eq)**4)))*g_eff*n_0**2 + n_0*((1+4*alpha*(torch.exp(N)/a_eq)**3)/(2*(1+(a_eq/torch.exp(N))+alpha*(torch.exp(N)/a_eq)**3)))*x_prime
...
#nets = [FCNN(n_input_units=3,  hidden_units=(256,128,64,), actv=nn.SiLU,) for _ in range(2)]

nets = [CustomNN(n_input_units=3, hidden_units= (256, 128, 64), actv = nn.SiLU, n_output_units = 1) for _ in range(2)]
#nn.SiLU
#nets = torch.load('nets_LCDM_np.ph')
...
tg0 = Generator1D(32, t_min=Om_m_min, t_max=Om_m_max)#, method='log-spaced-noisy')

vg0 = Generator1D(32, t_min=Om_m_min, t_max=Om_m_max)#, method='log-spaced')

tg1 = Generator1D(32, t_min=g_a_min, t_max=g_a_max)#, method='log-spaced-noisy')

vg1 = Generator1D(32, t_min=g_a_min, t_max=g_a_max)#, method='log-spaced')

train_gen = tgz ^ tg0 ^ tg1
...
solver = BundleSolver1D(ode_system=ODE_LCDM,
                        ...
                        theta_min=(Om_m_min,g_a_min),
                        theta_max=(Om_m_max,g_a_max),
                        eq_param_index=(0,1,),
                        optimizer=adam,
...
plt.savefig('loss_LCDM_mg.png')

# Save the neural network:
torch.save(solver._get_internal_variables()['best_nets'], 'nets_LCDM_mg.ph')